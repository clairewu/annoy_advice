...
exports.add = function(req,res,next){
	...
	reply.save(function(err){
		...
		Topic.findOne({_id:topic_id},function(err,topic){
			...
			//发送at消息
			at_ctrl.send_at_message(content,topic_id,reply.author_id);
		});
	});
	req.params.tid = topic_id;
	topic_ctrl.index(req,res,next);
...
function get_replies_by_query(query,cb){
	Reply.find(query,[],{sort:[['update_at','desc']]},function(err,replies){
		if(err) return next(err);
		if(replies.length == 0) return cb(err,[]);
...
exports.get_replies_by_query = get_replies_by_query;